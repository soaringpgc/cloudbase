      ...
      register_rest_route( $this->namespace, $this->resource_path, 
        array(	
          ...
          array (
             ...
             // Here we register our permissions callback. The callback is fired before the main callback to check if the current user can access the endpoint.
         	'permission_callback' => array($this, 'cloud_base_maintenance_access_check' ),        	 		      		
      	  )
      	)
      );	
	}
...
// 			}
	  ...
	  if( $wpdb->num_rows > 0 ) {	
    	  ...
    	  return new \WP_Error( 'No Equipment', esc_html__( 'no Equiptment avaliable.', 'my-text-domain' ), array( 'status' => 204 ) );
	   }
	...
	// annual_due_date
	  ...
	  if (!empty($request['captian'])){
	  	$the_user = get_user_by( 'ID', $request['captian_id'] ); 
			if (!$the_user) {
	  		 	return new \WP_Error( 'invalid user', esc_html__( 'Captian Not found.', 'my-text-domain' ), array( 'status' => 400 ) );		
	  	} else {
				$captian_id = $the_user->ID;
			}
	  }	  	  
...
//   				// read it back to get id and send
 	  			...
 	  			$items = $wpdb->get_row( $sql, OBJECT);
//  	  			return new \WP_Error( 'update_passsed', esc_html__( $items , 'my-text-domain' ), array( 'status' => 400 ) ); 	
 	  			return new \WP_REST_Response ($items);											
 			}	else {
