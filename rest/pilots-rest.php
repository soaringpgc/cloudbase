     ...
     register_rest_route( $this->namespace, $this->resource_path, 
        array(	
          ...
          array(
            ...
            // Here we register our callback. The callback is fired when this endpoint is matched by the WP_REST_Server class.
            'callback' => array( $this, 'cloud_base_pilots_put_edit_callback' ),
            // Here we register our permissions callback. The callback is fired before the main callback to check if the current user can access the endpoint.
         	'permission_callback' => array($this, 'cloud_base_private_access_check' ),),
          array (
...
// call back for pilots:	
	public function cloud_base_pilots_get_callback( \WP_REST_Request $request) {
			 return rest_ensure_response( 'Hello World, this is the Cloud Based Pilots REST API ' . $request['id']);
	...
	public function cloud_base_pilots_post_callback( \WP_REST_Request $request) {
		/* 
			Process your POST request here.
		*/
	}
	public function cloud_base_pilots_put_callback( \WP_REST_Request $request) {
		/* 
